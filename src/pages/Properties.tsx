...
export default function Properties() {
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        const response = await fetch('http://demorealestate.iceiy.com/wp-json/wp/v2/property?_embed', {
          headers: {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
            {loading ? (
              ...
              Array.from({ length: 6 }).map((_, idx) => (
                <div key={idx} className="bg-white rounded-2xl overflow-hidden shadow-xl border border-gold/10 animate-pulse">
                  ...
                  <div className="p-8 flex-grow flex flex-col">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-8 py-4 border-y border-forest/5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs uppercase tracking-widest font-medium">{prop.bedrooms || '0'} Bedrooms</span>
                      </div>
                      <div className="flex items-center gap-2">
